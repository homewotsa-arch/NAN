...
import { 
  ...
  Menu, Utensils, Award, PhoneCall, Sparkles, ShoppingCart, MapPin, CreditCard, Mail
} from 'lucide-react';
...
const INITIAL_PRODUCTS = [
  ...
  },
  {
    ...
    isHalal: false,
    image: 'https://images.unsplash.com/photo-1595786802424-d6efbc413db5?auto=format&fit=crop&w=800&q=80',
...
export default function App() {
  ...
  const addToCart = (product) => {
    ...
    const isWaitlist = product.stockLeft <= 0;
  ...
  const hasWaitlistItems = cart.some(item => item.isWaitlist);

  const handleWhatsAppCheckout = (e) => {
    e.preventDefault();
    if (cart.length === 0) return;

    if (!customerName || !address || !pincode || (hasWaitlistItems && !gmail)) {
      alert('Please fill in your name, delivery address, pincode, and Gmail address for waitlist items.');
      return;
    }

    setProducts(prevProducts => {
      return prevProducts.map(prod => {
        const cartItemsForProd = cart.filter(item => item.id === prod.id && !item.isWaitlist);
        const totalQtyOrdered = cartItemsForProd.reduce((sum, item) => sum + item.qty, 0);
        
        const newStockLeft = Math.max(0, prod.stockLeft - totalQtyOrdered);
        return { ...prod, stockLeft: newStockLeft };
      });
    });

    let message = `*NEW ORDER / WAITLIST - NAN NAGA PICKLES*\n\n`;
    ...
    cart.forEach((item, index) => {
      message += `${index + 1}. *${item.name}* ${item.isWaitlist ? '(WAITLISTED)' : ''}\n   - Size: ${item.chosenWeight}\n   - Qty: ${item.qty}\n   - Price: ₹${item.price * item.qty}\n\n`;
    });
    message += `*Total Amount:* ₹${subtotal}\n\n`;
    message += `Please confirm my order/waitlist!`;
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
          {filteredProducts.map((product) => {
            ...
            const isAvailable = product.stockLeft > 0;
              ...
              >
                <div>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="text-xs font-semibold px-3 py-1.5 rounded border border-neutral-800 bg-neutral-950">
                      {isAvailable ? (
                        <span className="text-emerald-400">In Stock: {product.stockLeft} units remaining</span>
                      ) : (
                        <span className="text-amber-400">Out of Stock — Join Waiting Cart for restock notification</span>
                      )}
                    </div>
                ...
                <div className="p-6 pt-0">
                  <button
                    ...
                    className={`w-full py-3.5 rounded-xl font-bold transition flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] ${
                      isAvailable 
                  ...
                  >
                    ...
                    {isAvailable ? `Add to Cart (${currentOption.label} - ₹{currentOption.price})` : `Join Waiting Cart (${currentOption.label})`}
                  </button>
                </div>
              </div>
            );
                ...
                <>
                      ...
                      <div key={item.cartItemId} className="flex justify-between items-center bg-neutral-950 p-3 rounded-lg border border-neutral-800">
                        <div>
                          <h4 className="font-bold text-sm text-white flex items-center gap-2">
                            {item.name}
                            {item.isWaitlist && (
                              <span className="bg-amber-950/80 text-amber-400 text-[10px] px-2 py-0.5 rounded border border-amber-800 font-semibold">
                                Waiting Cart
                              </span>
                            )}
                          </h4>
                  ...
                  <form onSubmit={handleWhatsAppCheckout} className="py-4 space-y-4">
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      {hasWaitlistItems && (
                        <div>
                          <label className="block text-amber-400 mb-1 font-semibold flex items-center gap-1">
                            <Mail className="w-3.5 h-3.5" /> Gmail Address (Required for Waitlist Notification) *
                          </label>
                          <input 
                            type="email" 
                            required={hasWaitlistItems}
                            value={gmail}
                            onChange={(e) => setGmail(e.target.value)}
                            placeholder="e.g. rahul@gmail.com"
                            className="w-full bg-neutral-950 border border-amber-800/60 rounded-lg p-2.5 text-white focus:outline-none focus:border-amber-500"
                          />
                        </div>
                      )}
