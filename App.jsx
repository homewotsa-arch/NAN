...
// Integrated using your exact uploaded files
const LOGO_BASE64 = "image_8.png"; 
const HORNBILL_BASE64 = "image_7.png"; 
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans relative overflow-x-hidden">
      ...
      {(view === 'landing' || view === 'transitioning') && (
        <div className="fixed inset-0 z-50 bg-[#f7f5f0] flex flex-col items-center justify-center cursor-pointer select-none">
          <div 
            onClick={handleLogoClick}
            className="group flex flex-col items-center p-8 transition-transform duration-300 hover:scale-105"
          >
            <div className="w-64 h-64 md:w-80 md:h-80 rounded-full bg-white shadow-2xl border border-neutral-300/80 flex items-center justify-center p-4 relative overflow-hidden">
              <img 
                src={LOGO_BASE64} 
                alt="NAN Taste of Home Logo" 
                className="w-full h-full object-contain rounded-full drop-shadow-sm" 
              />
            </div>

            <p className="mt-6 text-neutral-600 text-sm font-semibold tracking-widest uppercase animate-pulse">
              Click logo to enter shop
            </p>
          </div>
          ...
          {view === 'transitioning' && (
            <div className="absolute inset-0 pointer-events-none overflow-hidden z-50">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="absolute animate-fly-hornills"
                  style={{
                    bottom: `-100px`,
                    left: `${10 + i * 16}%`,
                ...
                >
                  <img 
                    src={HORNBILL_BASE64} 
                    alt="Flying Hornbill" 
                    className="w-36 md:w-52 h-auto drop-shadow-2xl"
                    style={{ transform: `rotate(${-10 + (i % 3) * 8}deg)` }}
                  />
                </div>
              ))}
            </div>
      ...
      <style>{`
        @keyframes flyHornbills {
          ...
          100% {
            transform: translateY(-130vh) scale(1.15);
            opacity: 1;
          }
        }
      ...
      <div className={`${view !== 'shop' ? 'opacity-0 pointer-events-none hidden' : 'opacity-100 transition-opacity duration-700'} min-h-screen bg-neutral-950 text-neutral-100`}>
        <nav className="sticky top-0 z-40 bg-neutral-900/95 backdrop-blur-md border-b border-neutral-800">
          <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setView('landing')}>
              <div className="w-10 h-10 rounded-full bg-white p-1 flex items-center justify-center border border-neutral-700 shadow">
                <img src={LOGO_BASE64} alt="Logo" className="w-full h-full object-contain rounded-full" />
              </div>
              <span className="text-sm font-bold tracking-wider text-neutral-200">NAN NAGA PICKLES</span>
            </div>

            <button 
              onClick={() => setIsCartOpen(true)}
              className="relative bg-red-600 hover:bg-red-700 text-white p-2.5 rounded-full transition flex items-center justify-center shadow-md"
            >
